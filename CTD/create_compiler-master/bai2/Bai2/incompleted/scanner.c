...
void skipComment() {  
  ...
  while(1)
  {
    ...
    if(charCodes [currentChar]==CHAR_TIMES) semaphore=1;
  ...
  if(semaphore!=2) error(ERR_ENDOFCOMMENT,lineNo,colNo);
  readChar();
  // printf("end\n");
}
...
Token* readConstChar(void) {
  ...
  token->string[1]='\0';//chi can luu 1 ki tu
  readChar();
  // readChar();
...
Token* getToken(void) {
  ...
  case CHAR_EXCLAIMATION:
    ...
    if((currentChar!=EOF)&&(charCodes[currentChar]==CHAR_EQ)){
      ...
      error(ERR_INVALIDSYMBOL,ln,cn);// bi loi vi ! ko hop le
      return token;
    }
  case CHAR_EQ:
