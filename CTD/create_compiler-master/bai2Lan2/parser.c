...
Type* compileLValue(void) {
  ...
  switch (var->kind) {
  case OBJ_CONSTANT: error(ERR_TYPE_INCONSISTENCY, lookAhead->lineNo, lookAhead->colNo); break;
  case OBJ_VARIABLE:
...
void compileAssignSt(void) {
  ...
  checkTypeEqualityAssign(varType[0],expType[0]);
  for(i=0;i<count;i++){
    checkTypeEqualityAssign(varType[i],expType[i]);
  }
}
